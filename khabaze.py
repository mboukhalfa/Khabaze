...
def ila_makhbaza(source_root, path, compiled_root, file_py, makhbaza_root):
    ...
    try:
        os.system(cmd)
        os.chdir(cwd)
        os.rename(source_root + makhbaza_root + file_py[:-3] + '.so',compiled_root + path + file_py[:-3] + '.so')
    except Exception as e:
        report['errors'] += 'error in ' + path + file_py + '\n'
...
if __name__ == '__main__':
    args, _ = getopt.getopt(sys.argv[1:],'hs:m:e:d:',['help','source_root=','main=', 'exclude=','dont_compile='])
    source_root = './'
    name = 'anastacia'
    for flag, arg in args:
        if flag in ('-h', '--help'): 
            print ('khabaze.py -s <source_root> -m <main_script> -e <exclude> -d <dont_compile>')
            sys.exit()
    ...
    #    if flag in ('-c', '--compiled_root'): compiled_root = arg.split(',')
